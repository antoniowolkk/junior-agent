...
- **Product:** billsplit — a library function that splits a shared bill's total into per-person shares.
- **Business outcome it exists to create:** correct, trustworthy money splitting.
- **Which lever it moves:** risk (a wrong split is a support ticket and a refund).
- **Primary users:** other code in the app that calls `split_bill`.
- **Definition of success:** shares always sum exactly to the input total, for any input.
...
- **Test framework:** none (plain `assert` in `test_billsplit.py`, run with `python3 test_billsplit.py`).
- **Database / storage:** none.

```
billsplit.py       the function
test_billsplit.py  its tests
...
| Purpose | Command |
| --- | --- |
| Run all tests | `python3 test_billsplit.py` |
