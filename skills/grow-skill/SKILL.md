...
| **This pack's own sessions** | This pack exists to help an agent write and use `AGENTS.md`-driven skills well — not to document its user's unrelated work. Grep only session transcripts whose `cwd` is this repo or one of its worktrees (`~/.claude/projects/-Users-*-<this-repo-name>*/*.jsonl`, or wherever this pack's own checkout lives), for friction with *this pack's own skills*: one that didn't trigger when it should have, routing that confused an agent, a step nobody could follow. Never grep transcripts from other repositories or other projects — a different codebase's bugs, migrations, or incidents are not this pack's research material. Never source a skill's content from another installed plugin's own behavior or output (a terse-mode formatter, an unrelated command pack) — that is a different tool, not evidence about skill authoring. Quote the pattern, never the content: no file contents, paths, credentials, or personal data enter the skill or the PR. |
